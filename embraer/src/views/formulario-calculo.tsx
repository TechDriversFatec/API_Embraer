...
function Calculo() {
  ...
  function manipularEnvio(evento: any) {
    ...
    if (formularioValido) {
      const selectFlapId = (
        document.getElementById("flap") as HTMLSelectElement
      ).options[
        (document.getElementById("flap") as HTMLSelectElement).selectedIndex
      ].value;
      const selectFrenagemId = (
        document.getElementById("frenagem") as HTMLInputElement
      ).value
        ? (document.getElementById("frenagem") as HTMLInputElement).value
        : "";
      const selectcondicaoId = (
        document.getElementById("condicaoPista") as HTMLInputElement
      ).value
        ? (document.getElementById("condicaoPista") as HTMLInputElement).value
        : "";
      ...
      if (success) {
        Axios.post("http://localhost:3002/salvarLog", {
          aeronave: (document.getElementById("aeronave") as HTMLSelectElement)
            .options[
            (document.getElementById("aeronave") as HTMLSelectElement)
              .selectedIndex
          ].text,
          motor: (document.getElementById("motor") as HTMLInputElement).value,
          certificacao: (
            document.getElementById("certificacao") as HTMLInputElement
          ).value,
          flap: (document.getElementById("flap") as HTMLSelectElement).options[
            (document.getElementById("flap") as HTMLSelectElement).selectedIndex
          ].text,
          condicaoPista: (
            document.getElementById("condicaoPista") as HTMLInputElement
  ...
  useEffect(() => {
    axios.get("http://localhost:3002/getAeronaves").then((response) => {
  ...
  const setAviaoCalculo = () => {
    const selectAviaoId = (
      document.getElementById("aeronave") as HTMLInputElement
    ).value
      ? (document.getElementById("aeronave") as HTMLInputElement).value
      : "";
    const selectFrenagemId = (
      document.getElementById("frenagem") as HTMLInputElement
    ).value
    ...
    if (selectFrenagemId != "" && selectcondicaoId != "") {
      axios
        .get(
          `http://localhost:3002/getFlap/${paramsToFlap.id}/${paramsToFlap.configuracao_freio}/${paramsToFlap.condicao_pista}`,
          {
            params: { paramsToFlap },
          }
        )
        .then((response) => {
          ...
          if (data.length > 0) {
            ...
            Swal.fire({
              ...
              html: " <b>There are no Flaps created for these settings</b> ",
            });
            console.log(data); // returns '[]'
          }
        });
    ...
    return (
      <>
        <Alert
          show={showStatus}
          variant="white"
          className="square border border-3"
          style={{ width: "100%" }}
        >
          <p id="alertP">Necessary to perform the landing:</p>
          <p id="resultP">{resultado}</p>
          <div className="d-flex justify-content-end botaoFecharAlert">
            <Button
              onClick={() => setShow(false)}
              variant="rounded btn btn-primary ml-2 float-end"
            >
              <b>Close</b>
            </Button>
          </div>
      ...
      <form id="form_criar" onSubmit={manipularEnvio}>
        <div className="card card-custom gutter-b">
          <div className="card-header">
            <h3 id="h3Calcular" className="card-title">
              Landing Calculation
            </h3>
                  ...
                  <div className="form-group col-lg-4 col-md-6 col-sm-12 sucess">
                    ...
                    >
                      ...
                      {avioes.map((aeronaveSelect) => (
                        <option
                          key={aeronaveSelect.id}
                          value={aeronaveSelect.id}
                        >
                          {aeronaveSelect.modelo}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="form-group col-lg-4 col-md-6 col-sm-12 sucess">
                    ...
                    >
                      ...
                      {flaps.map((flapRecebido) => {
                        return (
                          <option key={flapRecebido.id} value={flapRecebido.id}>
                            {flapRecebido.tipo_flap}
                          </option>
                        );
                      })}
                    </select>
                ...
                <div className="row">
                  ...
                  <div className="form-group col-lg-4 col-md-6 col-sm-12 sucess">
                    ...
                    <select
                      className="form-control select"
                      id="rev-inoperantes"
                      title="rev-inoperantes"
                      onChange={receberRev}
                    >
                      <option value="" disabled selected>
                        Select a Reverser Option
                      </option>
                  ...
                  <div className="form-group col-lg-4 col-md-6 col-sm-12 sucess">
                    ...
                    <select
                      ...
                      className="form-control select"
                      name="unidade"
                      onChange={receberUnidade}
                      id="unidade"
                    >
                      <option value="placeholder" disabled selected>
                        Select an Unite Measurement
                      </option>
